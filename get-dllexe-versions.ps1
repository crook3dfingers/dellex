...
echo "Recursively searching $path for all .dll and .exe files..."
$files = Get-ChildItem $path -recurse -include *.dll,*.exe

if ($files.Count -eq 0) {
	echo "No .dll or .exe files were found."
	return
}
...
echo "Writing file versions to $output..."

# Sort the array and remove duplicate values (rows containing the same name and version).
$uniqueArray = $array | Sort-Object -Property Name, Version -Unique

# Write the $uniqueArray into the supplied output file.
$uniqueArray | Export-Csv $output -NoTypeInformation
